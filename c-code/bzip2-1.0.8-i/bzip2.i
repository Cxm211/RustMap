...
const Int32 SM_I2O = 1;
const Int32 SM_F2O = 2;
const Int32 SM_F2F = 3;


const Int32 OM_Z = 1;
const Int32 OM_UNZ = 2;
const Int32 OM_TEST = 3;
...
static inline void ERROR_IF_EOF(int i) {
    if (i == 
# 225 "bzip2.c" 3 4
            (-1)
# 225 "bzip2.c"
...
static inline void ERROR_IF_NOT_ZERO(int i) {
    if (i != 0) ioError();
}

static inline void ERROR_IF_MINUS_ONE(int i) {
    if (i == (-1)) ioError();
}
...
static inline Bool ISFLAG(Cell* aa, const Char* s) {
...
# 1847 "bzip2.c"
                               ; aa = aa->link) {
      if (ISFLAG(aa, "--")) { decode = False; continue; }
...
# 1915 "bzip2.c"
                               ; aa = aa->link) {
      if (ISFLAG(aa, "--")) break;
      if (ISFLAG(aa, "--stdout")) srcMode = SM_F2O; else
      if (ISFLAG(aa, "--decompress")) opMode = OM_UNZ; else
      if (ISFLAG(aa, "--compress")) opMode = OM_Z; else
      if (ISFLAG(aa, "--force")) forceOverwrite = True; else
      if (ISFLAG(aa, "--test")) opMode = OM_TEST; else
      if (ISFLAG(aa, "--keep")) keepInputFiles = True; else
      if (ISFLAG(aa, "--small")) smallMode = True; else
      if (ISFLAG(aa, "--quiet")) noisy = False; else
      if (ISFLAG(aa, "--version")) license(); else
      if (ISFLAG(aa, "--license")) license(); else
      if (ISFLAG(aa, "--exponential")) workFactor = 1; else
      if (ISFLAG(aa, "--repetitive-best")) redundant(aa->name); else
      if (ISFLAG(aa, "--repetitive-fast")) redundant(aa->name); else
      if (ISFLAG(aa, "--fast")) blockSize100k = 1; else
      if (ISFLAG(aa, "--best")) blockSize100k = 9; else
      if (ISFLAG(aa, "--verbose")) verbosity++; else
      if (ISFLAG(aa, "--help")) { usage ( progName ); exit ( 0 ); }
         else
